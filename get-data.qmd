...
## 1. Leeds Boundary and Geographies

Get the Transport for the Leeds (Leeds) boundary and relevant administrative geographies.

```{r}
# Leeds Boundary (assuming local file availability or prior download)
# This part mirrors the logic in s3.qmd
leeds_path = "/home/robin/github/robinlovelace/counterbid/leeds_boundary/Leeds_Area_Boundary.shp"
if(file.exists(leeds_path)){
    leeds_boundary = read_sf(leeds_path)
    ...
    # Filter
    leeds_boundary = st_transform(leeds_boundary, st_crs(lads))
    msoas = st_transform(msoas, st_crs(lads))
    
    leeds_lads = lads[leeds_boundary, ]
    leeds_msoas = msoas[leeds_boundary, ]
    
    # Save
    write_sf(leeds_boundary, "leeds_boundary.geojson", delete_dsn = TRUE)
    write_sf(leeds_lads, "leeds_lads.gpkg", delete_dsn = TRUE)
    write_sf(leeds_msoas, "leeds_msoas.gpkg", delete_dsn = TRUE)
}
```

## 2. Motorway Network (Leeds England)

Download motorway data from OpenStreetMap.

```{r}
# Define the bounding box for Leeds England
bbox <- getbb("Leeds England, UK")
...
Download 5 years of road safety data (2020-2024) and filter to Leeds.
...
# Filter to Leeds if boundary exists
if(file.exists("leeds_boundary.geojson")) {
  leeds_boundary = read_sf("leeds_boundary.geojson")
  collisions_sf = stats19::format_sf(collisions_5y, lonlat = TRUE)
  collisions_sf = st_transform(collisions_sf, st_crs(leeds_boundary))
  collisions_leeds = collisions_sf[leeds_boundary, ]
  write_sf(collisions_leeds, "collisions_leeds_2020_2024.gpkg", delete_dsn = TRUE)
}
```
...
if(file.exists(od_zip_path)) {
  ...
  # Filter to Leeds region
  # We need the MSOA codes from the Leeds boundary
  if(file.exists("leeds_msoas.gpkg")) {
    leeds_msoas = read_sf("leeds_msoas.gpkg")
    valid_msoas = leeds_msoas$MSOA21CD
    
    # Filter: Keep trips where BOTH Origin and Destination are within Leeds
    ...
    od_leeds_wide = od_leeds |>
      pivot_wider(names_from = method, values_from = count, values_fill = 0)
    
    # Save
    write_csv(od_leeds_wide, "od_leeds_2021.csv")
    message("Saved od_leeds_2021.csv")
  } else {
    warning("leeds_msoas.gpkg not found. Skipping OD filtering.")
  }
} else {
...
# Filter for Leeds region
```

## 6. Leeds Transport Network (osmactive)
...
if(file.exists("leeds_boundary.geojson")) {
  ...
  # Get the travel network for Leeds
  # We use a broader region (e.g., Leeds England) and clip to the boundary
  osm_leeds = get_travel_network("England", boundary = leeds_boundary, boundary_type = "clipsrc")
  
  # Extract networks
  cycle_net_leeds = get_cycling_network(osm_leeds)
  drive_net_leeds = get_driving_network(osm_leeds)
  
  # Calculate distance to road and classify
  cycle_net_leeds = distance_to_road(cycle_net_leeds, drive_net_leeds)
  cycle_net_leeds = classify_cycle_infrastructure(cycle_net_leeds)
  
  # Save datasets
  write_sf(cycle_net_leeds, "leeds_cycling_network.gpkg", delete_dsn = TRUE)
  write_sf(drive_net_leeds, "leeds_driving_network.gpkg", delete_dsn = TRUE)
}
```
...
```{r}
files_to_upload = c(
  "leeds_boundary.geojson",
  "leeds_lads.gpkg",
  "leeds_msoas.gpkg",
  "south_east_motorways.gpkg",
  "collisions_leeds_2020_2024.gpkg",
  "od_leeds_2021.csv",
  "leeds_cycling_network.gpkg",
  "leeds_driving_network.gpkg",
